...
pub mod kv {
    ...
    ///
    /// # Warning
    /// If the stored value exists but cannot be deserialised as `i64`
    /// (e.g. it was written by a different schema), the counter resets to 0
    /// and a WARN log is emitted. The bad value is then overwritten.
    pub fn increment(key: &str, delta: i64) -> Result<i64, String> {
        let current: i64 = match get(key) {
            None => 0,
            Some(bytes) => match serde_json::from_slice::<i64>(&bytes) {
                Ok(v) => v,
                Err(e) => {
                    crate::log::warn(&format!(
                        "kv::increment: key '{key}' exists but is not a valid i64 \
                         (resetting to 0): {e}"
                    ));
                    0
                }
...
/// Define a tool plugin's WIT export wiring (cargo-component / wasm32 only).
///
/// This is a **thin convenience wrapper** around the pattern shown in the
/// `base64-tool` example.  Because `cargo-component` generates a `Guest`
/// trait whose associated types vary per-world, the full trait implementation
/// must be written by the plugin author inside a `#[cfg(target_arch = "wasm32")]`
/// block.  See `examples/base64-tool/src/lib.rs` for the recommended pattern.
///
/// > **Note:** This macro is currently a stub reserved for future use.
/// > It does **not** auto-generate the `Guest` impl.  Use the explicit
/// > `#[cfg(target_arch = "wasm32")] mod wasm_exports { … }` pattern instead.
#[macro_export]
macro_rules! define_tool {
    ($impl_ty:ty) => {
        // Reserved for future auto-generation of the cargo-component Guest impl.
        // Currently a no-op; see the base64-tool example for the explicit pattern.
    };
}
...
// ── Unit tests (non-wasm32 stub paths) ────────────────────────────────────────

#[cfg(test)]
mod tests {
    use super::*;

    // ── PluginResult convenience constructors ──────────────────────────────────
    #[test]
    fn plugin_result_ok() {
        let r = PluginResult::ok("hello");
        assert!(r.success);
    ...
    fn plugin_result_err() {
        let r = PluginResult::err("oops");
        assert!(!r.success);
        assert!(r.output.is_empty());
        assert_eq!(r.error.as_deref(), Some("oops"));
    }

    // ── HttpResponse body helpers ──────────────────────────────────────────────
    #[test]
    ...
    // ── kv stub: increment initialises from zero and respects delta ────────────
    //
    // On non-wasm32, kv::get always returns None so increment always
    // produces delta (since there is no backing store).  This verifies
    // the arithmetic and error path don't panic.
    #[test]
    fn kv_increment_stub_returns_delta() {
        // On host the backing store is a no-op, so effective current == 0.
        let result = kv::increment("test_counter", 5);
        // The set call on host also no-ops (returns Ok), so result == 5.
        assert_eq!(result.unwrap(), 5);
    }

    #[test]
    fn kv_get_json_returns_none_on_missing_key() {
        let v: Option<i64> = kv::get_json("nonexistent_key_xyzzy");
        assert!(v.is_none());
    }

    // ── clock: now_ms returns a plausible value on the host ───────────────────
    #[test]
    fn clock_now_ms_is_nonzero() {
        let ms = clock::now_ms();
        // Unix time in ms: any value > Jan 2020 (1577836800000) is sane.
        assert!(ms > 1_577_836_800_000, "clock::now_ms returned implausible value: {ms}");
    }

    #[test]
    fn clock_timezone_is_utc() {
        assert_eq!(clock::timezone(), "UTC");
    }

    // ── config stub: returns env var or None ──────────────────────────────────
    #[test]
    fn config_get_returns_none_for_missing_key() {
        // Very unlikely to be set in CI.
        let v = config::get("__PRX_PDK_TEST_KEY_DEFINITELY_NOT_SET__");
        assert!(v.is_none());
    }

    #[test]
    fn config_get_or_returns_default() {
        let v = config::get_or("__PRX_PDK_TEST_KEY_DEFINITELY_NOT_SET__", "fallback");
        assert_eq!(v, "fallback");
    }

    // ── MiddlewareAction serialisation ─────────────────────────────────────────
    #[test]
    fn middleware_action_continue_serialises() {
        let action = MiddlewareAction::Continue { data: r#"{"x":1}"#.to_string() };
    ...
    #[test]
    fn middleware_action_block_serialises() {
        let action = MiddlewareAction::Block { reason: "rate limit".to_string() };
        let json = serde_json::to_string(&action).unwrap();
        assert!(json.contains("\"action\":\"block\""));
        assert!(json.contains("rate limit"));
    }

    // ── ToolSpec roundtrip through JSON ───────────────────────────────────────
    #[test]
    fn tool_spec_json_roundtrip() {
        let spec = ToolSpec {
        ...
        assert_eq!(back.parameters_schema, r#"{"type":"object"}"#);
    }
}
