...
<section class="space-y-6">
  ...
  <div class="flex gap-1 rounded-lg border border-gray-700 bg-gray-800/50 p-1">
    <button
      ...
      class="rounded-md px-4 py-2 text-sm font-medium transition {activeTab === 'installed' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'}"
    >
    ...
    <button
      ...
      class="rounded-md px-4 py-2 text-sm font-medium transition {activeTab === 'discover' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'}"
    >
  ...
  <!-- Toast -->
  {#if toast}
    <div class="rounded-lg px-4 py-2 text-sm {toastType === 'error' ? 'border border-red-500/30 bg-red-500/10 text-red-300' : 'border border-green-500/30 bg-green-500/10 text-green-300'}">
      {toast}
    </div>
  {/if}
            ...
            <div class="mt-3 flex items-center justify-between">
              ...
              {#if confirmUninstall === skill.name}
                <div class="flex items-center gap-2">
                  <span class="text-xs text-yellow-400">{t('skills.confirmUninstall').replace('{name}', skill.name)}</span>
                  <button
                    type="button"
                    onclick={() => handleUninstall(skill.name)}
                    disabled={uninstallingName === skill.name}
                    class="rounded px-2 py-1 text-xs font-medium text-red-400 transition hover:bg-red-500/20 disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    class="rounded px-2 py-1 text-xs text-gray-400 transition hover:bg-gray-700"
                  >
                    {t('common.no')}
                  </button>
                </div>
              {:else}
                <button
                  type="button"
                  onclick={() => handleUninstall(skill.name)}
                  class="rounded px-2 py-1 text-xs text-red-400 transition hover:bg-red-500/20"
                >
                  {t('skills.uninstall')}
                </button>
              {/if}
  ...
  {#if activeTab === 'discover'}
    <div class="flex flex-col gap-3 sm:flex-row">
      <select
        ...
        class="rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-gray-200"
      >
      ...
      <input
        ...
        class="flex-1 rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-gray-200 placeholder-gray-500 focus:border-sky-500 focus:outline-none"
      />
      <button
        type="button"
    ...
    {#if discoverLoading}
      <p class="text-sm text-gray-400">{t('skills.searching')}</p>
          ...
          <article class="rounded-xl border border-gray-700 bg-gray-800 p-4">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0 flex-1">
                <h3 class="truncate text-lg font-semibold text-gray-100">{result.name}</h3>
                <p class="text-xs text-gray-500">{t('skills.owner')} {result.owner}</p>
              </div>
              <span class="rounded-full border border-gray-600 bg-gray-700 px-2 py-0.5 text-xs text-gray-300">
                {result.source}
              </span>
            </div>
            {#if result.description}
              <p class="mt-2 line-clamp-3 text-sm text-gray-400">{result.description}</p>
            {/if}
            <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-gray-500">
              ...
              {/if}
              {#if result.language}
                <span class="rounded bg-gray-700 px-1.5 py-0.5">{result.language}</span>
              {/if}
              <span class="{result.has_license ? 'text-green-400' : 'text-yellow-400'}">
            ...
            <div class="mt-3 flex items-center justify-between">
              ...
              {#if alreadyInstalled}
                <span class="rounded-full border border-green-500/50 bg-green-500/20 px-2 py-1 text-xs font-medium text-green-300">
                  {t('skills.installed')}
                </span>
              {:else}
                <button
