...
<section class="space-y-6">
  ...
  {#if loading}
    <p class="text-sm text-gray-400">{t('channels.loading')}</p>
      ...
      {#each channels as channel}
        <article class="rounded-xl border border-gray-700 bg-gray-800 p-4">
          <div class="flex items-start justify-between gap-3">
            <h3 class="text-lg font-semibold text-gray-100">{channelLabel(channel.name)}</h3>
            <span
          ...
          <p class="mt-3 text-sm text-gray-400">{t('channels.type')}: {channelLabel(channel.type)}</p>
        </article>
      {/each}
    </div>
  {/if}
