...
use crate::config::files::{build_split_tables, read_merged_toml_with_gate};
...
pub struct SelfSystemConfig {
    ...
    /// Path to evolution config file (replaces OPENPRX_EVOLUTION_CONFIG / ZEROCLAW_EVOLUTION_CONFIG)
    #[serde(default)]
    pub evolution_config_path: Option<String>,
...
fn set_proxy_env_pair(key: &str, value: Option<&str>) {
    ...
    if let Some(value) = value.and_then(|candidate| normalize_proxy_url_option(Some(candidate))) {
        // SAFETY: Called during single-threaded config initialization before any
        // concurrent HTTP clients are created or read these env variables.
        // `set_proxy_env_vars` is invoked from `Config::apply_to_env`, which is
        // called only during startup before the async runtime spawns worker threads
        // that could observe the environment. No concurrent readers exist at this
        ...
        // SAFETY: Same single-threaded initialization context as the set branch
        // above — no concurrent readers of these env vars exist during startup.
        unsafe {
...
pub struct MemoryConfig {
    ...
    #[serde(default)]
    pub lucid_cmd: Option<String>,

    /// Token budget for lucid (replaces ZEROCLAW_LUCID_BUDGET / OPENPRX_LUCID_BUDGET)
    #[serde(default)]
    pub lucid_budget: Option<u32>,

    /// Recall timeout in ms (replaces OPENPRX_LUCID_RECALL_TIMEOUT_MS)
    #[serde(default = "default_lucid_recall_timeout_ms")]
    pub lucid_recall_timeout_ms: u64,

    /// Store timeout in ms (replaces OPENPRX_LUCID_STORE_TIMEOUT_MS)
    #[serde(default = "default_lucid_store_timeout_ms")]
    pub lucid_store_timeout_ms: u64,

    /// Local hit threshold: minimum number of local results before skipping
    /// remote Lucid recall. (replaces OPENPRX_LUCID_LOCAL_HIT_THRESHOLD)
    #[serde(default = "default_lucid_local_hit_threshold")]
    pub lucid_local_hit_threshold: usize,

    /// Failure cooldown in ms (replaces OPENPRX_LUCID_FAILURE_COOLDOWN_MS)
    #[serde(default = "default_lucid_failure_cooldown_ms")]
...
const fn default_lucid_store_timeout_ms() -> u64 {
    3000
}
const fn default_lucid_local_hit_threshold() -> usize {
    3
}
const fn default_lucid_failure_cooldown_ms() -> u64 {
    60_000
}
...
pub struct ScopeRule {
    ...
    /// Assignment whitelist: which `prx chat` sessions this scope may hand work
    /// to through `POST /api/chat-sessions/{id}/assign`.
    ///
    /// Entries are `{label}:{session_id}` — the same two-segment shape as
    /// `send_allow`, with the live chat session in the place of the recipient.
    /// Either segment may be `"*"`, and a bare `"*"` matches every session. The
    /// label is the human name the chat process registered under, so
    /// `"workstation:*"` follows that chat across restarts while
    /// `"*:{session_id}"` names exactly one run of it.
    ///
    /// **The label is self-declared by whoever registered the session.** It is a
    /// convenience for writing rules, not an identity assertion: anything that
    /// can reach the register endpoint can claim any label. Where that matters,
    /// name the session id, which the daemon generates.
    ///
    /// Assignment is **default-deny**: with no `assign_owners` and no
    /// `assign_allow` anywhere, no channel correspondent can assign anything.
    /// That is the opposite of `send_allow`, whose empty default still permits
    /// the turn's own channel, and it is deliberate — an assignment starts work
    /// on another entry point, so silence must mean "no".
    #[serde(default)]
    pub assign_allow: Vec<String>,
    /// Assignment blacklist, in the same `{label}:{session_id}` form.
    ///
    /// Evaluated across *every* matching rule, so a later `assign_deny` can
    /// never be shadowed by an earlier rule, and it outranks both
    /// `assign_allow` and `autonomy.scopes.assign_owners` — an owner named here
    /// is refused, exactly as `tools_deny` outranks `tools_allow`.
    ///
    /// Unlike `send_deny` there is no alias folding: both segments are compared
    /// literally. A chat session id is generated by this daemon and has exactly
    /// one spelling, so there is no alias to fold and no guess to make.
    #[serde(default)]
    pub assign_deny: Vec<String>,
}
...
pub struct ScopeConfig {
    ...
    /// Correspondents who own this daemon and may hand work to any `prx chat`
    /// session it knows about.
    ///
    /// Entries are `{channel}:{sender}` — the same two-segment shape
    /// `send_allow` uses for a destination, read here as an *origin*. Either
    /// segment may be `"*"`; a bare `"*"` makes every correspondent an owner
    /// and is almost never what an operator wants.
    ///
    /// The identity compared here is the runtime-injected `_zc_scope`, not
    /// anything a model or an HTTP body claimed, so an entry names a real
    /// platform account. An owner still loses to a matching `assign_deny`.
    ///
    /// Empty (the default) means no correspondent owns this daemon, so the only
    /// way to assign work is an authenticated operator-plane request.
    #[serde(default)]
...
impl ScopeConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        for (index, rule) in self.rules.iter().enumerate() {
            for (field, entries) in [("send_allow", &rule.send_allow), ("send_deny", &rule.send_deny)] {
                for entry in entries {
                    validate_acl_pair_entry(
                        &format!("autonomy.scopes.rules[{index}].{field}"),
                        "channel",
                        "recipient",
                        entry,
                    )?;
            ...
            for (field, entries) in [("assign_allow", &rule.assign_allow), ("assign_deny", &rule.assign_deny)] {
                for entry in entries {
                    validate_acl_pair_entry(
                        &format!("autonomy.scopes.rules[{index}].{field}"),
                        "label",
        ...
        for entry in &self.assign_owners {
            validate_acl_pair_entry("autonomy.scopes.assign_owners", "channel", "sender", entry)?;
        }
        self.validate_no_shadowed_allow("send_allow", |rule| &rule.send_allow)?;
        self.validate_no_shadowed_allow("assign_allow", |rule| &rule.assign_allow)
    }

    /// Reject a whitelist an earlier rule makes unreachable.
    ///
    /// Both `send_allow` and `assign_allow` are first-match-wins, so a narrower
    /// rule written below a broader one would take effect nowhere and say
    /// nothing about it. Their deny counterparts sweep every matching rule and
    /// therefore cannot be shadowed, which is why only the allow side is
    /// checked here.
    fn validate_no_shadowed_allow(&self, field: &str, entries: impl Fn(&ScopeRule) -> &Vec<String>) -> Result<()> {
        for (index, rule) in self.rules.iter().enumerate() {
            if entries(rule).is_empty() {
                continue;
            }
            let shadow = self
                .rules
                .iter()
                .take(index)
                .position(|earlier| !entries(earlier).is_empty() && scope_criteria_cover(earlier, rule));
            if let Some(shadow) = shadow {
                anyhow::bail!(
                    "autonomy.scopes.rules[{index}].{field} can never apply: rules[{shadow}]                      matches every context rules[{index}] matches and already carries a {field}                      whitelist, so only that earlier whitelist is ever consulted. Move                      rules[{index}] above rules[{shadow}], narrow rules[{shadow}], or merge the                      entries into rules[{shadow}].{field}."
                );
            }
        }
        Ok(())
...
/// Reject a two-segment ACL entry that could never match anything.
///
/// One validator for every `{left}:{right}` list — outbound recipients,
/// assignable chat sessions, assignment owners — because an entry that cannot
/// match is a silent fail-open on a deny list, and catching it in one place is
/// what stops a new list from quietly skipping the check.
fn validate_acl_pair_entry(path: &str, left_name: &str, right_name: &str, entry: &str) -> Result<()> {
    let trimmed = entry.trim();
    if trimmed.is_empty() {
        anyhow::bail!("{path} must not contain an empty entry");
    }
    if trimmed == "*" {
        return Ok(());
    }
    let Some((left, right)) = trimmed.split_once(':') else {
        anyhow::bail!("{path} entry {trimmed:?} must be \"*\" or \"{{{left_name}}}:{{{right_name}}}\"");
    };
    if left.trim().is_empty() {
        anyhow::bail!("{path} entry {trimmed:?} has an empty {left_name} segment");
    }
    if right.trim().is_empty() {
        anyhow::bail!("{path} entry {trimmed:?} has an empty {right_name} segment");
    }
    Ok(())
}
...
pub struct RuntimeConfig {
    ...
    /// Codex stream idle timeout in seconds (replaces ZEROCLAW_CODEX_STREAM_IDLE_TIMEOUT_SECS)
    #[serde(default)]
...
async fn resolve_runtime_config_dirs(
    ...
    // CLI --config-dir takes highest priority
    if let Some(dir) = explicit_config_dir {
        let dir = dir.trim();
        if !dir.is_empty() {
            let openprx_dir = PathBuf::from(dir);
            return Ok((
                openprx_dir.clone(),
...
/// Scan all config secret fields for legacy `enc:` values and migrate them to the secure
/// `enc2:` (ChaCha20-Poly1305) format in-place, then persist the updated config to disk.
///
/// This is called once at startup so that secrets which have never been accessed are
/// upgraded eagerly instead of remaining in the vulnerable XOR-cipher format indefinitely.
/// Individual failures are logged as warnings and do not abort the migration.
pub(crate) async fn migrate_config_legacy_secrets(config: &Config) -> Result<()> {
    let openprx_dir = config
        .config_path
        .parent()
        .context("Config path must have a parent directory")?;
    let store = crate::security::SecretStore::new(openprx_dir, config.secrets.encrypt);

    // Re-read the raw TOML to obtain the still-encrypted forms of secrets (the loaded
    // `config` struct already has them decrypted to plaintext).
    let raw_toml = fs::read_to_string(&config.config_path).await.with_context(|| {
        format!(
            "Failed to read config for legacy-secret migration: {}",
            config.config_path.display()
        )
    })?;

    let mut raw_config: Config =
        toml::from_str(&raw_toml).context("Failed to parse raw config for legacy-secret migration")?;
    raw_config.config_path = config.config_path.clone();
    raw_config.workspace_dir = config.workspace_dir.clone();

    // Migrate the fixed (non-agent) secret fields.
    let mut fixed: Vec<Option<String>> = vec![
        raw_config.api_key.clone(),
        raw_config.composio.api_key.clone(),
        raw_config.browser.computer_use.api_key.clone(),
        raw_config.web_search.brave_api_key.clone(),
        raw_config.storage.provider.config.db_url.clone(),
    ];
    let mut total_migrated = store.migrate_legacy_secrets(&mut fixed)?;
    raw_config.api_key = fixed.get(0).cloned().flatten();
    raw_config.composio.api_key = fixed.get(1).cloned().flatten();
    raw_config.browser.computer_use.api_key = fixed.get(2).cloned().flatten();
    raw_config.web_search.brave_api_key = fixed.get(3).cloned().flatten();
    raw_config.storage.provider.config.db_url = fixed.get(4).cloned().flatten();

    // Migrate per-agent api_key fields.  Use a sorted key list to avoid relying on
    // HashMap iteration order when mapping results back.
    let agent_keys: Vec<String> = {
        let mut keys: Vec<String> = raw_config.agents.keys().cloned().collect();
        keys.sort();
        keys
    };
    let mut agent_values: Vec<Option<String>> = agent_keys
        .iter()
        .filter_map(|k| raw_config.agents.get(k).map(|a| a.api_key.clone()))
        .collect();

    total_migrated += store.migrate_legacy_secrets(&mut agent_values)?;

    for (key, new_val) in agent_keys.iter().zip(agent_values) {
        if let Some(agent) = raw_config.agents.get_mut(key) {
            agent.api_key = new_val;
        }
    }

    if total_migrated == 0 {
        tracing::debug!("No legacy enc: secrets found — skipping migration write");
        return Ok(());
    }

    tracing::info!(
        count = total_migrated,
        "Migrated legacy XOR-encrypted secrets to ChaCha20-Poly1305; saving config"
    );

    // Serialize and persist.  The values are already in enc2: form so we must NOT
    // call to_stored_toml_value() (which would double-encrypt them).  Serialize
    // directly and write atomically.
    let toml_str =
...
impl Config {
    ...
    pub async fn load_or_init_with_config_dir(explicit_config_dir: Option<&str>) -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            // Eagerly migrate any remaining legacy enc: secrets so they are upgraded
            // even if they have never been accessed since the XOR-cipher era.
            if let Err(e) = migrate_config_legacy_secrets(&config).await {
                tracing::warn!(error = %e, "Legacy secret migration encountered an error");
            }
            Ok(config)
        } else {
...
"#;
    ...
    async fn resolve_runtime_config_dirs_uses_env_workspace_first() {
        ...
        let (config_dir, resolved_workspace_dir, source) =
            resolve_runtime_config_dirs(&default_config_dir, &default_workspace_dir, None)
                .await
                .unwrap();
...
"#;
    ...
    // ── Assignment ACL validation (plan c1) ─────────────────────────────────

    fn assign_rule(channel: Option<&str>, assign_allow: &[&str], assign_deny: &[&str]) -> ScopeRule {
        ScopeRule {
            channel: channel.map(str::to_string),
    ...
    /// The same malformed-entry check the outbound lists get, on the assignment
    /// lists. A `{label}:{session}` entry that cannot match is a silent
    /// fail-open on `assign_deny`, so it is refused at load time.
    ///
    /// MUTATION GUARD: stop validating the assignment lists in
    /// `ScopeConfig::validate` and this test fails.
    #[test]
    async fn scope_config_rejects_malformed_assignment_entries() {
        let colonless = ScopeConfig {
            rules: vec![assign_rule(None, &[], &["workstation"])],
            ..ScopeConfig::default()
        };
        let error = colonless
            .validate()
            .expect_err("a colon-less assign_deny entry can never match");
        let error = error.to_string();
        assert!(error.contains("assign_deny"), "{error}");
        assert!(error.contains("{label}:{session}"), "{error}");

        let empty_half = ScopeConfig {
            rules: vec![assign_rule(None, &["workstation:"], &[])],
            ..ScopeConfig::default()
        };
        assert!(
            empty_half.validate().is_err(),
            "an empty session segment matches nothing"
        );

        let owners = ScopeConfig {
            assign_owners: vec!["wacli".to_string()],
            ..ScopeConfig::default()
        };
        let error = owners.validate().expect_err("an owner entry needs both segments");
        let error = error.to_string();
        assert!(error.contains("assign_owners"), "{error}");
        assert!(error.contains("{channel}:{sender}"), "{error}");
    }

    /// `assign_allow` is first-match-wins exactly like `send_allow`, so a
    /// whitelist an earlier rule already covers is dead configuration and is
    /// rejected rather than silently ignored.
    ///
    /// MUTATION GUARD: drop the `assign_allow` call to
    /// `validate_no_shadowed_allow` and this test fails.
    #[test]
    async fn scope_config_rejects_a_shadowed_assign_allow() {
        let cfg = ScopeConfig {
            rules: vec![
                assign_rule(None, &["*"], &[]),
                assign_rule(Some("wacli"), &["workstation:*"], &[]),
            ],
            ..ScopeConfig::default()
        };
        let error = cfg.validate().expect_err("a shadowed assign_allow must be rejected");
        let error = error.to_string();
        assert!(error.contains("rules[1].assign_allow"), "{error}");
        assert!(error.contains("rules[0]"), "{error}");
    }

    /// A rule with only an `assign_deny` shadows nothing: deny sweeps every
    /// matching rule, so there is nothing for it to swallow.
    #[test]
    async fn scope_config_accepts_an_assign_allow_below_a_deny_only_rule() {
        let cfg = ScopeConfig {
            rules: vec![
                assign_rule(None, &[], &["*:some-session"]),
                assign_rule(None, &["workstation:*"], &[]),
            ],
            ..ScopeConfig::default()
        };
        cfg.validate()
            .expect("a deny-only rule does not shadow an assign_allow");
    }

    #[test]
    async fn scope_config_accepts_well_formed_assignment_entries() {
        let cfg = ScopeConfig {
            rules: vec![assign_rule(Some("wacli"), &["workstation:*", "*"], &["*:dead-session"])],
            assign_owners: vec!["wacli:+15550001111".to_string(), "*:root".to_string()],
            ..ScopeConfig::default()
        };
