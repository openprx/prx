...
fn encode_sasl_plain(nick: &str, password: &str) -> String {
    ...
    for chunk in bytes.chunks(3) {
        ...
        // SAFETY: (triple >> N & 0x3F) is always in 0..64, and CHARS has 64 elements
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(&mut writer, &format!("USER {} 0 * :OpenPRX", self.username)).await?;
...
mod tests {
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("prx"));
