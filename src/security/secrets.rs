...
// in `~/.openprx/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.openprx/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(openprx_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: openprx_dir.join(".secret_key"),
            enabled,
        }
    }
