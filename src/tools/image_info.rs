...
impl Tool for ImageInfoTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Resolve the path within the workspace and canonicalize to block symlink escapes
        ...
        let resolved_path = match tokio::fs::canonicalize(&full_path).await {
            ...
            Err(e) => {
                return Ok(ToolResult {
                    success: false,
                    output: String::new(),
                    error: Some(format!("File not found: {path_str} ({e})")),
                });
            }
        };
...
mod tests {
    ...
    async fn execute_real_file() {
        ...
        let dir = std::env::temp_dir().join("openprx_image_info_test");
        ...
        let png_bytes: Vec<u8> = vec![
            ...
            0x08, 0xD7, 0x63, 0xF8, 0xCF, 0xC0, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0xE2, 0x21, 0xBC, 0x33, // CRC
    ...
    async fn execute_with_base64() {
        let dir = std::env::temp_dir().join("openprx_image_info_b64");
        let _ = tokio::fs::create_dir_all(&dir).await;
